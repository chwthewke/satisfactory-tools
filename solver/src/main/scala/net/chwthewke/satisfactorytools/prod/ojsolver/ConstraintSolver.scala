...
object ConstraintSolver {
  final case class Error( msg: String ) extends Throwable( msg, null, false, false )

  def apply[F[_]: Sync]: Solver[F] = new Solver[F] {
    override def solve( bill: Bill, recipes: RecipeSelection ): F[Solution] =
      Sync[F].interruptible( Pure.solve( bill, recipes ) ).flatMap( _.leftMap( Error ).liftTo[F] )
  }

  private object Pure extends Solver[Either[String, *]] {
