...
case class PowerGenerator(
  ...
  def itemClassName: Option[ClassName] =
    Option
      .when( className.name.startsWith( PowerGenerator.buildingPrefix ) )(
        PowerGenerator.itemPrefix + className.name.stripPrefix( PowerGenerator.buildingPrefix )
      )
      .map( ClassName( _ ) )
}
...
object PowerGenerator {
  ...
  private val itemPrefix: String     = "Desc_"
