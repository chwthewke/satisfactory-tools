...
final case class Countable[N, A]( item: A, amount: N ) {
  def map[B]( f: A => B ): Countable[N, B] = Countable( f( item ), amount )
  def flatMap[B]( f: A => Countable[N, B] )( implicit N: Numeric[N] ): Countable[N, B] =
    f( item ).mapAmount( N.times( amount, _ ) )
...
object Countable {
  ...
  implicit def countableInstance[N](
  ...
  private class CountableInstance[N]( implicit N: Numeric[N] )
    ...
    override def flatMap[A, B]( fa: Countable[N, A] )( f: A => Countable[N, B] ): Countable[N, B] = fa.flatMap( f )
